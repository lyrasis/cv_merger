...
if File.file?($cv_merger_path)
  CSV.foreach($cv_merger_path,
    ...
    $cv_merger_data << row.to_hash
  end
end

$cv_merger_data.each_with_index do |data, idx|
  row_num = idx + 2
  begin
    ...
    raise NotFoundException, "[Row #{row_num}] Unable to find enumeration '#{enumeration}'" unless enum
    ...
    unless old_enum_value
      raise NotFoundException, "[Row #{row_num}] Unable to find old_value '#{old_value}' in enumeration '#{enumeration}'"
    end

    unless new_enum_value
      raise NotFoundException, "[Row #{row_num}] Unable to find new_value '#{new_value}' in enumeration '#{enumeration}'"
    end

    if old_value == new_value
      raise Exception, "[Row #{row_num}] Unable to merge '#{old_value}' into itself"
    end

    $stdout.puts "[Row #{row_num}] Merging '#{old_value}' into '#{new_value}' for '#{enumeration}'"
    RequestContext.open(
